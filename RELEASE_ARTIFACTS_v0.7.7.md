# ALi Imagine v0.7.7 release artifacts

## macOS installer

```text
ALi-Imagine_0.7.7_aarch64.dmg
```

Public release:

```text
https://github.com/tomislavrupic/ALi-Imagine-site/releases/tag/v0.7.7
```

Size:

```text
11,037,262 bytes
```

SHA-256:

```text
4f20a6480d38a3fc36c4973f9b67bf156843f407dd6e9393a26e17a473627020
```

Verification:

- DMG CRC validation passed.
- Installer contains ALi-Imagine 0.7.7 and an Applications shortcut.
- Embedded application and bundled WARP sidecar pass deep, strict code-sign verification.
- 53 frontend tests and 160 deterministic Rust tests passed; 35 explicitly gated real-model tests remained ignored.
- Isolated first launch created a fresh database without touching the user's normal projects or models.
- Existing projects, outputs, app-managed models, external model libraries, and Concept LoRAs are not bundled or modified.
- The application is ad-hoc signed and not Apple-notarized; first launch may require Control-click **Open** once.
